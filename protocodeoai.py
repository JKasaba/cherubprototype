...
def load_data(json_path="law_office_data.json"):
    ...
    # loop through all clients and add their details
...
# query the openai agent
def ask_question(vector_store, query):
    llm = ChatOpenAI(model_name="gpt-4", temperature=0)  # ✅ Fixed to use Chat Model
    qa = RetrievalQA.from_chain_type(llm, chain_type="stuff", retriever=vector_store.as_retriever())
    return qa.invoke(query)  # ✅ Fixed for LangChain Deprecation Warning
